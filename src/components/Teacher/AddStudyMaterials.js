...
function AddStudyMaterials() {
  ...
  const SubmitHandler = async (e) => {
    ...
    try {
      const response = await Axios.post("http://localhost:8081/AddStudyMaterialsFilesystem", formData, {
        headers: {
    ...
    <div>
      ...
      <div className="center-content">
        ...
        <br />
          ...
          <div className={styles.formgroup}>
            <label>image</label>
            <input  type="file" name="thumbnail" id="file" accept="image/*" required/>
          </div>
          <div className={styles.formgroup}>
            <label>File</label>
            <input type="file" name="pdf_file" id="file" accept="pdf/*" required/>
          </div>
      ...
      <ul style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', padding: 0, listStyleType: 'none' }}>
          {courses.map((course) => (
            // <li key={course._id}>
